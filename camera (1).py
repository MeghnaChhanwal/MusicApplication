...
class VideoCamera(object):
    ...
    def get_frame(self):
        ...
        df1 = df1[['Name', 'Artist', 'Link']]
        df1 = df1.head(15)
        
        current_time = time.time()  # Current time
        time_elapsed = current_time - self.start_time  # Calculate elapsed time

        mood_detected = False  # Flag to check if mood detected
        ...
        for (x, y, w, h) in face_rects:
            ...
            cv2.putText(image, emotion_dict[maxindex], (x + 20, y - 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255),
                        2, cv2.LINE_AA)
            df1 = music_rec()
            mood_detected = True  # Set flag as mood is detected
        ...
        if mood_detected:
            cap1.stop()  # Stop the webcam stream if mood detected
            print("Mood detected:", emotion_dict[show_text[0]])
            # Perform actions to display the emotion or print the page
